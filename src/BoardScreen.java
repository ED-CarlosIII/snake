...
public class BoardScreen extends JPanel {
    ...
    public void setUpPlayers() {
        ...
        if (0 < returnMaxPlayers()) {
            getPlayers().get(0).añadirColorJugador(Color.green);
        }
        if (1 < returnMaxPlayers()) {
            getPlayers().get(1).añadirColorJugador(Color.blue);
        }
        if (2 < returnMaxPlayers()) {
            getPlayers().get(2).añadirColorJugador(Color.red);
        }
    ...
    public BoardScreen(MainWindow mw) {
        ...
        whichPlayer.setText(players.get(currPlayer).devolverNombre());
        ...
        roll.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                getPlayers().get(getCurrPlayer()).aumentarPuntosJugador(1);

                for (Player p : getPlayers()) {
                    if (p.devolverPosicion() >= getX() * getY() - 1) {
                        getSuccess().setText("And the winner is: " + p.devolverNombre() + "\nYour score: " + p.devolverPuntuacionJugador());
                ...
                getWhichPlayer().setText(getPlayers().get(getCurrPlayer()).devolverNombre());
