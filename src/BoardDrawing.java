...
public class BoardDrawing extends JPanel {
    ...
    /**
     * Numero de filas que tiene el entorno de juego
     * @param row
     * Numero de columnas que tiene el entorno de juego
     * @param col
     * Carga un objeto BoradScreen que contiene los componentes graficos de la pantala del juego.
     * @param bs
     */
    ...
    public void paintComponent(Graphics g) {
        ...
        int xOffset = obtenerXOffSet(width, cellWidth);
        int yOffset = obtenerYOffSet(height, cellHeight);
        ...
        for (Rectangle cell : cells) {
            ...
            for (int pl = 0; pl < bs.maxPlayers; pl++) {
                if (bs.players.get(pl).getPosition() == cellnos[i]) {                         //only one player considered here
                    ...
                    g2d.setColor(cambiarColorJuagador(pl));        //change to player color
    ...
    private int obtenerYOffSet(int height, int cellHeight) {
        return (height - (row * cellHeight)) / 2;
    }

    private int obtenerXOffSet(int width, int cellWidth) {
        return (width - (col * cellWidth)) / 2;
    }

    private Color cambiarColorJuagador(int pl) {
