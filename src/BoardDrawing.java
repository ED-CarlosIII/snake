...
public class BoardDrawing extends JPanel {
    ...
    /**
     *
     * @param row corresponde al numero de la fila
     * @param col corresponde al numero de la columna
     * @param bs
     */
    ...
    public void paintComponent(Graphics g) {
        ...
        CeldasVacias(xOffset, cellWidth, yOffset, cellHeight);
        ...
        for (Rectangle cell : cells) {
            ...
            Recorre1(i, g2d, cell, cellWidth, cellHeight);

            if (cellnos[i] == row * col - 1) {
                Recorre(i, g2d, cell, cellWidth, cellHeight);
            }
            i++;
        }
    ...
    private void CeldasVacias(int xOffset, int cellWidth, int yOffset, int cellHeight) {
        if (cells.isEmpty()) {
            for (int i = 0; i < row; i++) {
                for (int j = 0; j < col; j++) {
    ...
    private void Recorre1(int i, Graphics2D g2d, Rectangle cell, int cellWidth, int cellHeight) {
    ...
    private void Recorre(int i, Graphics2D g2d, Rectangle cell, int cellWidth, int cellHeight) {
        for (int pl = 0; pl < bs.maxPlayers; pl++) {
            if (bs.players.get(pl).getPosition() >= cellnos[i]) {                         //only one player considered here
    ...
    private void Colorea(Graphics2D g2d) {
        for (Rectangle cell : cells) {
            g2d.draw(cell);
        }
    }

    private void Rellena(Graphics2D g2d) {
        for (Rectangle cell : cells) {
            g2d.fill(cell);
        }
    }
    ...
    /**
     *
     * @param pnos Garantiza la posicion, en este caso 0.
     * @return Muestra el mensje correspondiente, depende de si es -1 o 1.
     */
     ...
     * @param pnos Muestra desde el numero.
     */
